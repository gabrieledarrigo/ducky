...
TEST should_return_the_related_error_t(void) {
    ...
    ASSERT_EQ(ERR_COMMAND_NOT_RECOGNIZED,  get_error_t(-3).code);
    ASSERT_STR_EQ("ERR_COMMAND_NOT_RECOGNIZED",  get_error_t(-3).message);

    ASSERT_EQ(ERR_MAX_DATA_SIZE,  get_error_t(-4).code);
    ASSERT_STR_EQ("ERR_MAX_DATA_SIZE",  get_error_t(-4).message);

    ASSERT_EQ(ERR_KEY_LENGTH, get_error_t(-5).code);
    ASSERT_STR_EQ("ERR_KEY_LENGTH", get_error_t(-5).message);

    ASSERT_EQ(ERR_NO_KEY, get_error_t(-6).code);
    ASSERT_STR_EQ("ERR_NO_KEY", get_error_t(-6).message);

    ASSERT_EQ(ERR_NO_DATA, get_error_t(-7).code);
    ASSERT_STR_EQ("ERR_NO_DATA", get_error_t(-7).message);
    PASS();
}
